...
type ResponseRestPowerPushOrderNotifyGin struct {
	...
	ArrivedAmount string  `form:"arrived_amount" json:"arrived_amount" uri:"arrived_amount" binding:"omitempty"` // 到账金额
	...
	FailReason    string  `form:"failReason" json:"failReason" uri:"failReason" binding:"omitempty"`             // 失败原因，有些渠道没有返回，不是很准确，但电费失败大部分原因都是户号不对或者地区不对或者缴费金额小于欠费金额
}
